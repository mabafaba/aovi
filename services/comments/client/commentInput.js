class CommentInput extends HTMLElement {
  constructor() {
    ...
    this.commentCategories = []; // Store comment categories from room data
    ...
    this.t = translator(this.translations, () => {}, this.shadowRoot);
  }
  ...
  render() {
    this.shadowRoot.innerHTML = `
        ...
        <div class="comment-categories" style="display: ${
          this.commentCategories.length > 0 ? "flex" : "none"
        };">
                ...
                ${this.commentCategories
                  .map(
                    ...
                    <div class="radio-option">
                        <input type="radio" id="category${index}" name="commentCategory" value="${category}" ${
                      index === 0 ? "checked" : ""
                    }>
    ...
    // Dispatch rendered event after rendering
    setTimeout(() => {
      // Manually trigger translation for newly rendered elements
  ...
  triggerTranslation() {
    // Manually trigger translation for elements in shadow DOM
    const lang = document.documentElement.lang || "en";

    // Translate text elements
    const elements = this.shadowRoot.querySelectorAll("[data-translator-text]");
    elements.forEach((element) => {
      const key = element.getAttribute("data-translator-text");
      if (this.translations[lang] && this.translations[lang][key]) {
        element.innerText = this.translations[lang][key];
      } else {
        element.innerText = key;
      }
    });

    // Translate attribute elements
    this.shadowRoot
      .querySelectorAll("[data-translator-]")
      .forEach((element) => {
        Array.from(element.attributes).forEach((attr) => {
          if (attr.name.startsWith("data-translator-")) {
            const key = attr.value;
  ...
  connectedCallback() {
    ...
    this.loadCommentCategories().then(() => {
      this.render();
    });
    ...
    // Event listeners will be added after render
    this.addEventListener("rendered", () => {
      this.shadowRoot
        .querySelector("button")
        .addEventListener("click", () => this.sendComment());
      this.shadowRoot
  ...
  disconnectedCallback() {
    this.shadowRoot
      .querySelector("button")
      .removeEventListener("click", () => this.sendComment());
  }

  async loadCommentCategories() {
    try {
      const response = await fetch(`/aovi/rooms/id/${this.room}`);
      if (response.ok) {
        const roomData = await response.json();
        this.commentCategories = roomData.data?.commentCategories || [];
        console.log("Loaded comment categories:", this.commentCategories);
      } else {
        console.warn("Failed to load room data for comment categories");
        this.commentCategories = [];
      }
    } catch (error) {
      console.error("Error loading comment categories:", error);
      this.commentCategories = [];
    }
  }
  ...
  async sendComment() {
    ...
    // Get selected comment category
    const selectedCategoryRadio = this.shadowRoot.querySelector(
      'input[name="commentCategory"]:checked'
    );
    const selectedCommentCategory = selectedCategoryRadio
      ? selectedCategoryRadio.value
      : null;

    // Add comment category to userCategories if selected
    const finalUserCategories = { ...userCategories };
    if (selectedCommentCategory) {
      finalUserCategories.commentCategory = selectedCommentCategory;
    }
      ...
      try {
        ...
        if (response.ok) {
          ...
          document.dispatchEvent(
            new CustomEvent("<comment-input>: posted successfully", {
              detail: body,
            })
          );
        } else {
          ...
          if (response.status === 401) {
            window.location.href =
              "/aovi/views/login?targeturl=" + window.location.pathname;
          }
